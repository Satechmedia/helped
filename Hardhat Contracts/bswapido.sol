...
// File: contracts/BswapCrowdsale.sol
...
contract IDOSale is AccessControl, Pausable, ReentrancyGuard {
    ...
    // The cap amount everyone can purchase IDO up to
    ...
    event SoftCapChanged(uint256 softCap);
    ...
    constructor(
        ...
        require(_hardCap > 0, "IDOSale: HARD_CAP_INVALID");
        require(_softCap > 0, "IDOSale: SOFT_CAP_INVALID");
    ...
    /**
     * @dev Set hard cap for each user
     */
    ...
    /**
     * @dev Set soft cap for each user
     */
    function setSoftCap(uint256 _softCap) external onlyOwner {
        softCap = _softCap;

        emit SoftCapChanged(_softCap);
    }
    ...
    function purchase(uint256 amount) external nonReentrant whenNotPaused {
        ...
        require(totalPurchasedAmount + amount <= hardCap, "IDOSale: HARD_CAP_EXCEEDED");
        ...
        uint256 purchaseTokenAmount = amount;
    ...
    function claim(uint256 amount) external nonReentrant whenNotPaused{
        ...
        uint256 claimedTokenAmount = amount * idoPrice;
        claimedAmounts[_msgSender()] += amount;
        ido.safeTransfer(_msgSender(), claimedTokenAmount);
    ...
    /**
     * @dev Users withdraw purchased tokens after token sale ended
     */
    function withdraw(uint256 amount) external nonReentrant whenNotPaused {
        require(amount > 0, "IDOSale: WITHDRAW_AMOUNT_INVALID");
        purchasedAmounts[_msgSender()] += amount;
        purchaseToken.safeTransfer(_msgSender(), amount);
     ...
     */
    function sweepPurchaseToken(address to) external onlyOwner {
    ...
    /**
     * @dev `Operator` sweeps `idoToken` from the sale contract to `to` address
     */
    function sweepIdoToken(address to) external onlyOwner {
        require(to != address(0), "IDOSale: ADDRESS_INVALID");
        require(endTime <= block.timestamp, "IDOSale: SALE_NOT_ENDED");
        uint256 bal = ido.balanceOf(address(this));
        ido.safeTransfer(to, bal);
